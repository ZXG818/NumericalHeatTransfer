    ...
    contains
        ...
        implicit none
            ...
            !让用户开始输入节点处的导热系数
            write(*, *)"请输入节点处的导热系数:"
            do i=2, N-1    !N为节点个数，只需要对中间节点赋物性值即可，边界节点不考虑。
                write(*, *) NodeGroup(i)%lambda
            end do
            !开始进行求解界面处的当量导热系数，
            !利用调和平均法来求解，使用均匀网格
            !正如之前所说，界面也只需要关心中间界面，问题边界的界面物性值不用考虑。
            do i=2, N-2
...
1002        inquire(file=filename, exist=alive)
            ...
            write(fileid, "(F8.4)") NodeGroup(N-1)%lambda
            close(fileid)
            return 
        end subroutine
        !
        ...
        implicit none
            ...
            N = size(SurfaceGroup, 1) - 1
            !生成AW信息
            AW(1) = 0                                 !边界条件
            do i=2, N
                AW(i) = SurfaceGroup(i)%lambda / delta_x
            end do
            !生成AE信息
            AE(N) = 0                                 !边界条件
            do i=1, N-1
                AE(i) = SurfaceGroup(i+1)%lambda / delta_x
            end do
            !
    ...
    contains
        ...
        implicit none
            ...
            !回代过程是从倒数第二个节点到第二个节点
            !先求倒数第二个节点的温度
            NodeGroup(length-1)%temperature = Q(length-2)
            do i=length-2, 2, -1
                NodeGroup(i)%temperature = P(i-1)*NodeGroup(i+1)%temperature + Q(i-1)
            end do
            return
        end subroutine
    end module
